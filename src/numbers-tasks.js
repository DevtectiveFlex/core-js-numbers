...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const vectorsLength = Math.sqrt((x1 ** 2 + y1 ** 2) * (x2 ** 2 + y2 ** 2));
  const dotProduct = x1 * x2 + y2 * y1;
...
function getFibonacciNumber(index) {
  ...
  for (let i = 3; i <= index; i += 1) {
    accumulator = prevNum + currentNum;
    prevNum = currentNum;
...
function getSumOfDigits(num) {
  const result = num
    .toString()
    .split('')
    .reduce((accumulator, value) => accumulator + parseInt(value, 10), 0);
  return result;
}
